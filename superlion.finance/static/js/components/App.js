...
class App extends Component {
    ...
    refreshPools = () => {
        ...
        fetch('https://superlion.finance/pools-v2.php')
            .then(response => response.json())
            .then((jsonData) => {
                // jsonData is parsed json object received from url
    ...
    */
    ...
    async getUserInfo(masterchef, pid, account) {
        ...
        const userInfo = await masterchefContract.methods.userInfo(pid, account).call()
    ...
    getFarmApy = (poolWeight, cakePriceUsd, poolLiquidityUsd) => {
    ...
    approveContract = (masterchef, type, tokenAddress) => {
        ...
        //this.state.SlionTokenContract.methods.approve(this.state.MasterChefContract._address, amount).send({
    ...
    deposit = (masterchef, pid, amount) => {
        ...
        let depositFunction = masterChefContract.methods.deposit(pid, amount);
    ...
    withdraw = (masterchef, _pid, amount) => {
        ...
        //this.setState({ loading: true })
        masterChefContract.methods.withdraw(_pid, amount).send({
            from: this.state.currentWallet,
            //gasLimit: 2000000
        }).on('confirmation', function(confirmationNumber, receipt) {
    ...
    render() {
        ...
        return ( <
            ...
            role = "alert" >
            <
            p > Earn < strong > MLION POOLS < /strong> rewards pools <strong>are active</strong > since block: # < a target = "_blank"
            href = "https://bscscan.com/block/countdown/7054000" > 7054000 < /a><br/ > < em > Sat May 01 2021 22: 57: 32 GMT + 0200 < /em></p >
            <
            /div>
